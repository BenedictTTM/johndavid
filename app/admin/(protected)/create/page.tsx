...
export default function CreatePostPage() {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (_d, vars) => {
            ...
            toast.success(isPublished ? 'Post published successfully!' : 'Draft saved');
            router.push('/admin');
        },
        onError: (err: Error) => {
            setSaveStatus('error');
    ...
    const handleSave = (published: boolean) => {
        ...
        if (featuredFile) formData.set('image', featuredFile);
    ...
    return (
        <div className="min-h-screen bg-white text-gray-900 flex flex-col font-sans selection:bg-[#FF6719]/20 selection:text-black">
            ...
            <header className="fixed top-0 inset-x-0 z-40 h-[50px] bg-white border-b border-gray-100 flex items-center px-4 sm:px-6 gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                        ...
                        className="flex items-center px-3.5 h-8 rounded-md border border-gray-200 text-[13px] font-medium text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-300 transition-all select-none"
                    >
                        Preview
                    </button>
            ...
            {/* ═══════════════════════════════════════════════════════════════
                FLOATING BOTTOM CONTROLS (As in Substack reference)
            ════════════════════════════════════════════════════════════════ */}
            ...
            <div className="fixed bottom-5 left-14 sm:left-6 z-30 flex items-center gap-1.5">
                <button
                    type="button"
                    onClick={() => toast.info('All revisions are saved automatically to your draft session.')}
                            ...
                            {imagePreview ? (
                                <div className="w-full relative">
                                    ...
                                    <button
                                        ...
                                        className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center rounded-full bg-white shadow-sm border border-gray-100 text-gray-500 hover:text-red-500 transition-colors"
                                    >
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="w-6 h-6 rounded flex items-center justify-center text-gray-400 hover:text-gray-700"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
